...
cowplot::plot_grid(p, marghisty, nrow = 1, rel_widths = c(1, 0.1))
```
...
This is the concept for *conditional distributions* and *conditional expectations*.

```{r condexp3, cache = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4, gganimate = list(rewind = FALSE), fig.cap = "test", include = knitr::is_html_output()}
plot_df %>%
...
ggplot(filter(plot_df, scene != "else")) +
  geom_rect(aes(xmin = as.numeric(scene) - 0.3, xmax = as.numeric(scene) + 0.3, 
                ymin = -Inf, ymax = Inf, fill = scene, group = scene), col = NA, alpha = 0.01) +
