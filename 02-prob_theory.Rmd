...
```{r condexp1, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4}
set.seed(112)
n <- 1000
xmax <- 5
x <- runif(n, -xmax, xmax)
...
```{r condexp2, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4}
...
```{r condexp3, cache = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4, gganimate = list(nframes = 30, fps = 30, rewind = FALSE), fig.cap = "The distribution of \\(X\\) conditional on different values of \\(Y\\).", include = knitr::is_html_output()}
plot_df %>%
...
ggplot(filter(plot_df, scene != "else")) +
  geom_rect(aes(xmin = as.numeric(scene) - 0.3, xmax = as.numeric(scene) + 0.3, 
                ymin = -Inf, ymax = Inf, fill = scene, group = scene), col = NA, alpha = 0.008) +
  geom_line(data = quadratic_df, aes(x, y)) +
  geom_boxplot(aes(as.numeric(scene), y, group = scene, col = scene),
               outlier.shape = NA, width = 0.5) +  
  geom_hline(yintercept = 8, linetype = "dashed", col = "grey70") +
  annotate("text", -4.8, 9, label = "E(X)", col = "grey70") +
  geom_point(aes(x, y, col = scene)) +
  geom_point(data = select(plot_df, x, y), aes(x, y), alpha = 0.2, col = "black") +
  theme(legend.position = "none") +
  labs(x = "Y", y = "X") +
  transition_states(scene, state_length = 10, transition_length = 1) +
  enter_fade() +
  exit_fade() +
...
```{r condexp4, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 3.33, fig.width = 5, out.width = "100%"}
