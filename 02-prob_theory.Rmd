...
A *measure* $\mu$ is a non-negative real valued function defined on a $\sigma$-algebra, i.e. $\mu:\cF\to\bbR_{\geq 0}$, where $\bbR_{\geq 0}$ are the non-negative real numbers and $\cF$ a $\sigma$-algebra of subsets of $\Omega$. The measure $\mu$ satisfies the following properties:
...
```{r wraparoundmeasureanim, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 2, gganimate = list(nframes = 30 * 3.5, fps = 30, rewind = TRUE), fig.cap = "An interval in [0,1] shifted by some fixed amount, with wrap-around, should have consistent length.", include = knitr::is_html_output()}
k <- 60
plot_df <- tibble(
